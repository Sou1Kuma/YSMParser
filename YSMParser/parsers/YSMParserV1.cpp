...
std::vector<uint8_t> YSMParserV1::getDecryptedData() {
    return m_decryptedData;
}

void YSMParserV1::saveToDirectory(std::string output_directory)
{
...
void YSMParserV1::parse() {
    ...
    while (offset < m_size)
    {
        ...
        std::vector<uint8_t> aesKey(m_buffer.get() + offset, ip + offset + 16);
        offset += 16;
        ...
        auto decrypted = aesDecrypt(aesKey, ivBytes, encryptedData);
